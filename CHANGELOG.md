...
- **Multi-Scale Species-Area Aggregation panel with per-group species columns + supporting materialized views** — Added a new panel for true species-area curve fitting with the `sars` R package, plus two supporting MVs. The previous "Site x Species Matrix" panel was misnamed — uniform 0.1° cells don't have enough area dynamic range for a meaningful SAR fit, so it was renamed to "Site x Species Matrix (0.1° grid)" with an honest description listing what it actually IS good for (beta diversity, hotspot mapping, accumulation curves). The new "Multi-Scale Species-Area Aggregation (SAR-ready)" panel aggregates herp observations across 5 scales (0.1° / 0.2° / 0.4° / 0.8° / 1.6°) giving a ~256× area dynamic range, with **per-group species count columns broken out** for fitting separate SAR curves per taxonomic group: `total_species`, `amphibia_species`, `reptilia_species`, `anura_species` (frogs), `caudata_species` (salamanders), `testudines_species` (turtles), `serpentes_species` (snakes). Cell areas are computed as **geodesic surface area on the WGS84 ellipsoid** via `ST_Area(geometry::geography)`, so they correctly account for Earth curvature and meridian convergence at higher latitudes. Backed by two new MVs: `mv_herpetofauna_sar_grid` (base, 0.1° cells with per-cell taxon_id arrays + per-group sub-arrays via `array_agg(...) FILTER`) and `mv_herpetofauna_sar_data` (pre-aggregated multi-scale data with per-group species counts at every (scale, cell, quality_grade) tuple). The pre-aggregation reduces panel query time from ~60 s (inline scan + aggregation against the 233 M-row observations table) to ~130 ms (sequential scan of the pre-aggregated MV). Combined MV size: ~108 MB (sar_grid) + ~135 MB (sar_data) = ~243 MB. Both MVs are added to `db/init/03-materialized-views.sql` (stubs) and `db/scripts/create-materialized-views.sql` (per-import build + atomic swap), and a one-shot `scripts/activate-sar-mv.sh` is provided for existing installs to build them without waiting for the next import.
